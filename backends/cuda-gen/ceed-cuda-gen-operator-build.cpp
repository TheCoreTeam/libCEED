...
// Setup fields
//------------------------------------------------------------------------------
static int CeedOperatorBuildKernelFieldData_Cuda_gen_low_order(std::ostringstream &code, CeedOperator_Cuda_gen *data, Tab &tab, CeedInt i,
                                                               CeedOperatorField op_field, CeedQFunctionField qf_field, FieldReuse_Cuda field_reuse,
                                                               CeedInt max_dim, CeedInt Q, CeedInt Q_1d, bool is_input, bool is_all_tensor,
                                                               bool is_at_points, bool use_3d_slices, bool skip_active_load) {
  bool      is_tensor = true, is_active = true;
  CeedBasis basis;
  ...
  switch (eval_mode) {
    ...
    case CEED_EVAL_INTERP:
      ...
      if (use_previous_field && !skip_active_load) {
        std::string reuse_var = "r_B" + ((field_reuse.is_input ? "_in_" : "_out_") + std::to_string(field_reuse.index));

        code << tab << "auto r_B" << var_suffix << " = " << reuse_var << ";\n";
      } else {
        bool is_collocated = false;
...
// Restriction
//------------------------------------------------------------------------------
static int CeedOperatorBuildKernelRestriction_Cuda_gen_low_order(std::ostringstream &code, CeedOperator_Cuda_gen *data, Tab &tab, CeedInt i,
                                                                 CeedInt field_input_buffer[], CeedOperatorField op_field,
                                                                 CeedQFunctionField qf_field, CeedInt max_dim, CeedInt Q_1d, bool is_input,
    ...
    if (field_input_buffer[i] != i) {
      ...
      switch (rstr_type) {
        ...
        case CEED_RESTRICTION_STRIDED: {
          ...
          code << tab << "const CeedInt strides" << var_suffix << "_0 = " << strides[0] << ", strides" << var_suffix << "_1 = " << strides[1]
               << ", strides" << var_suffix << "_2 = " << strides[2] << ";\n";
          code << tab << "ReadLVecStrided" << (is_all_tensor ? max_dim : 1) << "dLowOrder<num_comp" << var_suffix << ", " << P_name << ", strides"
               << var_suffix << "_0, strides" << var_suffix << "_1, strides" << var_suffix << "_2>(data, elem, d" << var_suffix << ", r_e"
               << var_suffix << ");\n";
          break;
        }
...
// Basis
//------------------------------------------------------------------------------
static int CeedOperatorBuildKernelBasis_Cuda_gen_low_order(std::ostringstream &code, CeedOperator_Cuda_gen *data, Tab &tab, CeedInt i,
                                                           CeedOperatorField op_field, CeedQFunctionField qf_field, CeedInt max_dim, CeedInt Q_1d,
                                                           bool is_input, bool is_all_tensor, bool is_at_points, bool use_3d_slices) {
  bool      is_tensor = true, is_collocated = true;
  CeedBasis basis;
    ...
    switch (eval_mode) {
      ...
      case CEED_EVAL_INTERP:
        if (is_at_points) {
          ...
          std::string function_name = (dim == 1 ? "Interp" : "InterpTensor") + std::to_string(dim) + "dLowOrder";
          ...
          // TODO: fix this
          if (is_collocated || !is_all_tensor) {
            return CEED_ERROR_INCOMPATIBLE;
          }
          // std::string function_name = is_tensor ? ((dim == 1 ? "Interp" : "InterpTensor") + std::string(is_collocated ? "CollocatedNodes" : "") +
          //                                          std::to_string(dim) + "d" + (is_all_tensor ? "" : "Flattened"))
          ...
          std::string function_name = is_tensor ? ((dim == 1 ? "Interp" : "InterpTensor") + std::string(is_collocated ? "CollocatedNodes" : "") +
                                                   std::to_string(dim) + "d" + (is_all_tensor ? "LowOrder" : "Flattened"))
                                                : "InterpNonTensor";
      ...
      case CEED_EVAL_GRAD:
        if (is_at_points) {
          ...
          std::string function_name =
              (dim > 1 ? "InterpTensor" : "Interp") + std::string(is_collocated ? "CollocatedNodes" : "") + std::to_string(dim) + "dLowOrder";
    ...
    switch (eval_mode) {
      ...
      case CEED_EVAL_INTERP:
        ...
        if (is_at_points) {
          // TODO: fix this
          return CEED_ERROR_INCOMPATIBLE;
          std::string function_name = (dim == 1 ? "InterpTranspose" : "InterpTransposeTensor") + std::to_string(dim) + "dLowOrder";
...
// QFunction
//------------------------------------------------------------------------------
static int CeedOperatorBuildKernelQFunction_Cuda_gen_low_order(std::ostringstream &code, CeedOperator_Cuda_gen *data, Tab &tab, CeedInt max_dim,
                                                               CeedInt max_num_points, CeedInt num_input_fields, CeedOperatorField *op_input_fields,
                                                               CeedQFunctionField *qf_input_fields, CeedInt num_output_fields,
                                                               CeedOperatorField *op_output_fields, CeedQFunctionField *qf_output_fields,
                                                               std::string qfunction_name, CeedInt Q_1d, bool is_all_tensor, bool is_at_points,
    ...
    for (CeedInt i = 0; i < num_input_fields; i++) {
      ...
      switch (eval_mode) {
        ...
        case CEED_EVAL_GRAD:
          ...
          // code << tab << "GradColloSlice3d<num_comp" << var_suffix << ", " << Q_name << ", OP_T_1D>(data, q, r_q" << var_suffix << ", s_G"
          //      << var_suffix << ", r_s" << var_suffix << ");\n";
          // code << tab << "GradColloSlice3dLowOrder<num_comp" << var_suffix << ", " << Q_name << ">(data, q, smem, r_G" << var_suffix << ", r_s"
          //      << var_suffix << ");\n";
          code << tab << "GradColloSlice3dLowOrder<num_comp" << var_suffix << ", " << Q_name << ">(data, q, smem, r_q" << var_suffix << ", r_G"
               << var_suffix << ", r_s" << var_suffix << ");\n";
          break;
        case CEED_EVAL_WEIGHT:
          code << tab << "CeedScalar r_s" << var_suffix << "[1];\n";
...
//------------------------------------------------------------------------------
static int CeedOperatorBuildKernel_Cuda_gen_low_order(CeedOperator op, bool *is_good_build) {
  ...
  for (CeedInt i = 0; i < num_input_fields; i++) {
    CeedCallBackend(CeedOperatorBuildKernelFieldData_Cuda_gen_low_order(code, data, tab, i, op_input_fields[i], qf_input_fields[i],
                                                                        input_matrix_reuse[i], max_dim, Q, Q_1d, true, is_all_tensor, is_at_points,
                                                                        use_3d_slices, false));
  }
  code << "\n" << tab << "// Output field constants and basis data\n";
  for (CeedInt i = 0; i < num_output_fields; i++) {
    CeedCallBackend(CeedOperatorBuildKernelFieldData_Cuda_gen_low_order(code, data, tab, i, op_output_fields[i], qf_output_fields[i],
                                                                        output_matrix_reuse[i], max_dim, Q, Q_1d, false, is_all_tensor, is_at_points,
                                                                        use_3d_slices, false));
  }
  ...
  for (CeedInt i = 0; i < num_input_fields; i++) {
    ...
    // ---- Restriction
    CeedCallBackend(CeedOperatorBuildKernelRestriction_Cuda_gen_low_order(code, data, tab, f, field_rstr_in_buffer, op_input_fields[f],
                                                                          qf_input_fields[f], max_dim, Q_1d, true, is_all_tensor, is_at_points,
                                                                          use_3d_slices));

    // ---- Basis action
    CeedCallBackend(CeedOperatorBuildKernelBasis_Cuda_gen_low_order(code, data, tab, f, op_input_fields[f], qf_input_fields[f], max_dim, Q_1d, true,
                                                                    is_all_tensor, is_at_points, use_3d_slices));
  }

  // -- Q function
  CeedCallBackend(CeedOperatorBuildKernelQFunction_Cuda_gen_low_order(code, data, tab, max_dim, max_num_points, num_input_fields, op_input_fields,
                                                                      qf_input_fields, num_output_fields, op_output_fields, qf_output_fields,
                                                                      qfunction_name, Q_1d, is_all_tensor, is_at_points, use_3d_slices));
  ...
  for (CeedInt i = 0; i < num_output_fields; i++) {
    ...
    // ---- Basis action
    CeedCallBackend(CeedOperatorBuildKernelBasis_Cuda_gen_low_order(code, data, tab, i, op_output_fields[i], qf_output_fields[i], max_dim, Q_1d,
                                                                    false, is_all_tensor, is_at_points, use_3d_slices));

    // ---- Restriction
    CeedCallBackend(CeedOperatorBuildKernelRestriction_Cuda_gen_low_order(code, data, tab, i, NULL, op_output_fields[i], qf_output_fields[i], max_dim,
...
extern "C" int CeedOperatorBuildKernel_Cuda_gen(CeedOperator op, bool *is_good_build) {
  ...
  if (CeedKernel_Cuda_low_order(data)) {
    CeedCallBackend(CeedOperatorBuildKernel_Cuda_gen_low_order(op, is_good_build));
    CeedCallBackend(CeedDestroy(&ceed));
    CeedCallBackend(CeedQFunctionDestroy(&qf));
    return CEED_ERROR_SUCCESS;
  }
